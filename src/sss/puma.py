...
def puma_crosswalk(path, year, nyc_wa_path=None):
    ...
    if ("797" in set(puma_txt["summary_level"])) and (
    ...
    crosswalk = crosswalk.merge(puma_pop, on="puma_code", suffixes=("", "_max"))
    # use the population of each row and
    #   population of puma to create a population weight
    crosswalk["weight"] = crosswalk["population"] / crosswalk["population_max"]
    ...
    if (
        ("53" in set(crosswalk["state_fips"])) or ("36" in set(crosswalk["state_fips"]))
    ) and (nyc_wa_path is None):
