...
def geo_identifier_creator(county_table, cpi_table):
    ...
    if df_combine.duplicated(['state', 'place']).sum() > 0:
        ...
        counties = df_combine.loc[df_combine.duplicated([
            'state', 'place'], keep=False), 'countyname'].values
        value_com = places + ' (' + counties + ')'
        df_combine.loc[df_combine.duplicated(
            ['state', 'place'], keep=False), 'place'] = value_com
    return df_combine
