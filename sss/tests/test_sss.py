...
def test_read_file():
    """ Test reading file """
    df, file = sss_table.read_file(os.path.join(DATA_PATH, "sss_data", 'AR2022_SSS_Full.xlsx'))
...
def test_check_extra_columns():
    ...
    df, file = sss_table.read_file(os.path.join(DATA_PATH, "sss_data", 'OR2021_SSS_ARPA_Full.xlsx'))
...
def test_check_extra_columns_error():
    ...
    with pytest.raises(ValueError, match="df should be a pandas dataframe."):
        sss_table.check_extra_columns(os.path.join(DATA_PATH, "sss_data", 'AR2022_SSS_Full.xlsx'))
...
def test_remove_rows(setup_and_teardown_package):
    ...
    data_folder_to_database(
        os.path.join(DATA_PATH, "sss_data", 'AR2022_SSS_Full.xlsx'),  db_file = db_file
    )
    result = session.query(SSS).filter(SSS.state == 'AR').all()
    assert len(result) == 4
